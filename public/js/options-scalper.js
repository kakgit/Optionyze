(function () {
    ...
    const mode = rawMode === "short" || rawMode === "dual" || rawMode === "covered" || rawMode === "options-scalper" ? rawMode : "long";
    ...
    const prefix = mode === "short"
        ? "rollingShortFutures"
        : ((mode === "dual" || mode === "covered" || mode === "options-scalper") ? "rollingDualFutures" : "rollingLongFutures");
    ...
    const endpointBase = mode === "short"
        ...
        : (mode === "dual"
            ...
            : (mode === "covered" ? "/api/covered-options" : (mode === "options-scalper" ? "/api/options-scalper" : "/api/rollingfutures-lt-long")));
    const modeLabel = mode === "short"
        ? "Short Mode"
        : (mode === "covered" ? "Covered Options" : (mode === "options-scalper" ? "Options-Scalper" : (mode === "dual" ? "Dual Mode" : "Long Mode")));
    const isCoveredMode = mode === "covered" || mode === "options-scalper";
    const isDualLikeMode = mode === "dual" || mode === "covered" || mode === "options-scalper";
    ...
    async function executeCoveredStrategies() {
        ...
        await checkConnection();
        if (!canUseLiveActions()) {
            throw new Error("Delta connection is not healthy enough to prepare the paper Options-Scalper strategy.");
        }
    ...
    async function placeManualOption(action, legSide, rowIndex) {
        ...
        if ((vAction !== "buy" && vAction !== "sell") || (vLegSide !== "ce" && vLegSide !== "pe")) {
            throw new Error("Option action and leg must be valid before placing a paper option order.");
        }
        if (manualOptionOrderInFlight) {
            throw new Error("A paper option order is already being processed. Please wait for it to finish.");
        }

        await checkConnection();
        if (!canUseLiveActions()) {
            throw new Error("Delta connection is not healthy enough to place a paper option order.");
        }
        ...
        if (!vExpiryDate) {
            throw new Error("Select an expiry date before placing a paper option order.");
        }
        if (!(vTargetDelta > 0)) {
            throw new Error("Enter a valid New D before placing a paper option order.");
        }
    ...
    ids.sellPeButton?.addEventListener("click", function () {
        void placeManualOption("sell", "pe", 1).then(function (objResult) {
            ...
            const vOrderId = String(objOrder.id || objOrder.order_id || "").trim();
            const vMessage = String(objResult?.message || "SELL PE paper option opened.").trim();
            if (trackedPayload) {
                renderOpenPositions(trackedPayload);
            }
    ...
    ids.sellCeButton?.addEventListener("click", function () {
        void placeManualOption("sell", "ce", 1).then(function (objResult) {
            ...
            const objOrder = objResult?.data?.order || {};
            const vOrderId = String(objOrder.id || objOrder.order_id || "").trim();
            const vMessage = String(objResult?.message || "SELL CE paper option opened.").trim();
            if (trackedPayload) {
                renderOpenPositions(trackedPayload);
            }
            setStatus(ids.pageStatus, vOrderId ? `${vMessage} Order ID: ${vOrderId}` : vMessage, "success");
    ...
    ids.buyCeButton?.addEventListener("click", function () {
        void placeManualOption("buy", "ce", 1).then(function (objResult) {
            ...
            const objOrder = objResult?.data?.order || {};
            const vOrderId = String(objOrder.id || objOrder.order_id || "").trim();
            const vMessage = String(objResult?.message || "BUY CE paper option opened.").trim();
            if (trackedPayload) {
                renderOpenPositions(trackedPayload);
            }
            setStatus(ids.pageStatus, vOrderId ? `${vMessage} Order ID: ${vOrderId}` : vMessage, "success");
    ...
    ids.buyPeButton?.addEventListener("click", function () {
        void placeManualOption("buy", "pe", 1).then(function (objResult) {
            ...
            const objOrder = objResult?.data?.order || {};
            const vOrderId = String(objOrder.id || objOrder.order_id || "").trim();
            const vMessage = String(objResult?.message || "BUY PE paper option opened.").trim();
            if (trackedPayload) {
                renderOpenPositions(trackedPayload);
            }
            setStatus(ids.pageStatus, vOrderId ? `${vMessage} Order ID: ${vOrderId}` : vMessage, "success");
    ...
    ids.execStrategyButton?.addEventListener("click", function () {
        void (isCoveredMode ? executeCoveredStrategies() : executeStrategy(1)).then(function (objResult) {
            ...
            const vMessage = String(objResult?.message || "Exec Strategy opened paper option position(s).").trim();
    ...
    ids.refreshOpenPositionsButton?.addEventListener("click", function () {
        void reconcileOpenPositions().then(function (objResult) {
            setStatus(ids.pageStatus, objResult?.message || "Paper open positions refreshed from Delta prices.", "success");
    ...
    ids.killSwitchButton?.addEventListener("click", function () {
        const confirmed = window.confirm(isCoveredMode
            ? "Kill switch will close all saved paper option positions using live Delta prices. Continue?"
    ...
    ids.clearOpenPositionsButton?.addEventListener("click", function () {
        const confirmed = window.confirm("Clear all imported open positions from the Open Positions section only? No paper close will be recorded.");
        if (!confirmed) {
            return;
        }
    ...
    ids.openPositionsBody?.addEventListener("click", function (event) {
        ...
        if (closeButton instanceof HTMLButtonElement) {
            ...
            const confirmed = window.confirm(`Close ${row.contractName || "this paper position"} now using the latest Delta price?`);
            if (!confirmed) {
                return;
            }
            void closeImportedOpenPosition(row).then(function (objResult) {
                ...
                const objOrder = objData.order || {};
                const vOrderId = String(objOrder.id || objOrder.order_id || "").trim();
                const vMessage = objResult?.message || "Paper position closed using live Delta price.";
                const trackedPayload = objResult?.data?.trackedOpenPositions || null;
                setStatus(ids.pageStatus, vOrderId ? `${vMessage} Order ID: ${vOrderId}` : vMessage, vTone);
                if (trackedPayload) {
                    renderOpenPositions(trackedPayload);
    ...
    void Promise.resolve().then(function () {
        ...
        setStatus(
            ids.pageStatus,
            error instanceof Error ? error.message : (isCoveredMode ? "Unable to load Options-Scalper page." : "Unable to load live futures page."),
            "danger"
        );
    });
